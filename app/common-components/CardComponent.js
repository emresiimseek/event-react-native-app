        ...
        return (
            ...
            <View style={styles.cardContainer}>
                <View style={styles.header}>
                    ...
                    <Icon name="ellipsis-v" style={{ marginRight: 10 }} color="gray" size={18} />
                </View>
                <View style={styles.body}>
                    <ImageBackground style={{ width: "100%", height: "100%" }} source={{ uri: "https://reactjs.org/logo-og.png" }} />

                </View>
                <View style={styles.footer}>
                    <View style={{ flex: 1, justifyContent: 'space-around', }}>
                        <Text style={{ fontWeight: "bold" }}>{event.activityTitle}</Text>
                        <Text style={{ fontSize: 10 }}><Icon name="users" size={15} /> 15 Kişi  Katılıyor</Text>
                    </View>
                    <View style={{ flexDirection: 'column', justifyContent: 'space-around', alignItems: 'flex-end' }}>
                        <Text>{moment(event.activityDate).format('LLL')} <Icon name="calendar" /></Text>
                        <Text>{event.categories.map(cat => <Text key={cat.id}>{cat.title}</Text>)} <Icon name="tag" /></Text>
                    </View>
                </View>
            </View >
        );
